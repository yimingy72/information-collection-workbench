...
- 第一行是紧凑查询表单和最近查询。
- 第二块是单个结果卡片。
- 结果卡头展示企业、状态、查询用时、紧凑的 ICP 缓存/实时企业数、运行中停止按钮和Excel导出。
- 投资关系与 ICP 结果实时进入现有分段表格，不新增大面积日志面板；完成后保持最终分页位置可用。
- “对外投资 / ICP备案”使用分段控件切换。
- 表格滚动区与分页区分离；分页始终固定在卡片底部且可见。

### 子域名查询

- 顶部使用紧凑分段控件切换“输入域名 / 选择 ICP 结果”。
- 枚举方式（被动来源 / DNS 字典 / 智能变体 / HTTP 探测）和开始按钮固定在输入区第二行，当前主域名数量紧邻主操作。
- 结果区以主域名、候选、已处理、有效子域名和阶段进度建立信息层级。
- 结果搜索与“全部 / 有响应 / 泛解析”筛选位于表格上方；结果筛选使用延迟值，SSE 结果批量合并，表格与分页沿用全站模式。
