...
数据源请求始终直连目标数据源。SeaMoon 仅用于 ICP 阶段；云函数未部署、未验证或已停止时，ICP 使用直连路由。
...
- **测试**：只验证链路，不切换路由。
- **停止**：关闭平台到 SeaMoon 的 ICP 路由，不删除云端函数。
...
- 数据源请求不会使用 SeaMoon；云函数代理只用于 ICP 查询。
