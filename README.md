...
- ICP 云函数路由按**实际页面请求数**计数，每5次请求进入下一代 YMICP/SeaMoon 会话；创宇盾拦截时对当前页最多重建2次隧道后重试。
...
SeaMoon 每个上游 HTTP 代理连接建立一个 WebSocket 隧道。ICP 每5次实际页面请求会重建 YMICP 分页会话和 SeaMoon 隧道；使用同一个函数地址不保证每次请求获得固定出口IP。若必须获得可验证的不同出口，需要配置多个云函数/出口或云 NAT/EIP，单个 FC 地址无法由应用层强制换公网IP。
