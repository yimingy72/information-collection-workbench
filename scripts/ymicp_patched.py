...
class beian:
    def __init__(self):
        ...
        # A short-lived session affinity cache keeps all pages of one company
        # on the same aiohttp connection. With SeaMoon this also keeps the same
    ...
    async def get_session(self, proxy=""):
        if proxy and proxy.startswith(("socks4://", "socks5://", "socks://")) and ProxyConnector:
            ...
            async def _request_without_proxy(*args, **kwargs):
                kwargs.pop("proxy", None)
                return await original_request(*args, **kwargs)

            session._request = _request_without_proxy
            try:
                yield session
            finally:
                try:
    ...
    async def _close_page_session_state(self, state):
        try:
    ...
    async def _get_page_session(self, session_key, proxy=""):
        ...
        async with self._page_sessions_lock:
            for key, state in list(self._page_sessions.items()):
                expired = now - state["last_used"] > self._page_session_ttl
                proxy_changed = key == session_key and state["proxy"] != proxy
                if (expired or proxy_changed) and not state["lock"].locked():
                    stale.append(self._page_sessions.pop(key))

            state = self._page_sessions.get(session_key)
            if state is None:
                context = self.get_session(proxy)
                ...
                self._page_sessions[session_key] = state

            state["last_used"] = now

            if len(self._page_sessions) > self._page_session_limit:
                candidates = sorted(
                    (item for item in self._page_sessions.items() if item[0] != session_key
                     and not item[1]["lock"].locked()),
                    key=lambda item: item[1]["last_used"],
                )
                while len(self._page_sessions) > self._page_session_limit and candidates:
                    key, old_state = candidates.pop(0)
                    if self._page_sessions.pop(key, None) is old_state:
                        stale.append(old_state)

        for old_state in stale:
            await self._close_page_session_state(old_state)
        return state

    async def _close_page_session(self, session_key, expected_state=None):
        state = None
        async with self._page_sessions_lock:
            current = self._page_sessions.get(session_key)
            if current is not None and (expected_state is None or current is expected_state):
                state = self._page_sessions.pop(session_key)
    ...
    async def getbeian(self, name, sp, pageNum, pageSize, proxy="", session_key=""):
        ...
        async def query_with_session(session, cached_auth=None):
            if use_captcha:
                ...
                if auth is None:
                    ...
                    auth = {
                        ...
                        "token": token,
                        "sign": sign,
                        "base_header": auth_header.copy(),
                    }
                p_uuid = auth["uuid"]
                token = auth["token"]
                sign = auth["sign"]
                base_header = auth["base_header"].copy()
                ...
                async with session.post(
                    ...
                    return True, await req.text(), auth
            ...
            sign = auth["sign"]
            base_header = auth["base_header"].copy()
            base_header.update({"token": token, "sign": sign})
            async with session.post(
        ...
        try:
            if state is not None:
                async with state["lock"]:
                    success, res, auth = await query_with_session(
                        state["session"], state.get("auth")
                    )
                    if success:
                        state["auth"] = auth
                ...
                async with self.get_session(proxy) as session:
                    success, res, auth = await query_with_session(session)
            ...
            p_uuid = auth.get("uuid", "")
            token = auth.get("token", "")
            sign = auth.get("sign", "")
            base_header = auth.get("base_header", {}).copy()
            current_ip = None
            active_session = state["session"] if state is not None else None
            if active_session is not None and hasattr(active_session, '_connector'):
                connector = active_session._connector
                if hasattr(connector, '_local_addr') and connector._local_addr:
                    current_ip = connector._local_addr[0]

            if "当前访问疑似黑客攻击" in res:
                if current_ip:
                    await self._add_blocked_ip(current_ip)
                elif not proxy and self.local_ipv6_addresses and self._last_used_ipv6_index >= 0:
                    blocked_ip = self.local_ipv6_addresses[self._last_used_ipv6_index]
                    await self._add_blocked_ip(blocked_ip)
                if state is not None:
                    await self._close_page_session(session_key, state)
    ...
    # 网站备案查询
    async def ymWeb(self, name, pageNum="", pageSize="", proxy="", session_key=""):
        try:
            success, data = await self.getbeian(
                name, 0, pageNum, pageSize, proxy, session_key
            )
            if not success:
                return {"code": 500, "message": data}
            ...
            return {"code": 122, "message": "查询失败", "error": str(exc)}
    ...
    async def cleanup(self):
        """清理资源"""
        async with self._page_sessions_lock:
            states = list(self._page_sessions.values())
